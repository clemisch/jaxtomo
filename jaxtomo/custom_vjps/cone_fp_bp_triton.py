...
from jaxtomo.projectors import cone_bp_triton
...
def get_fp_bp(angles, X, dX, Y, U, dU, V, dV, S, D):
    @jax.custom_vjp
    def project(vol):
        proj = cone_fp_triton.get_fp(vol, angles, dX, U, dU, V, dV, S, D)
        return proj
    ...
    def backproject(proj):
        vol = cone_bp_triton.get_bp(proj, angles, dU, dV, X, Y, dX, S, D)
        return vol
